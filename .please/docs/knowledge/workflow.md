...
> Defines the development workflow conventions for soop please.
...
```bash
bun run dev          # Watch mode (CLI)
bun run mcp <file>   # MCP server
```
...
bun run test packages/<pkg>/tests/<file>.test.ts    # Single file
bun run test -t "test name"                         # By test name
...
- Files: `*.test.ts` in `packages/*/tests/`

### Integration Testing

- Test complete user flows
- Verify data transactions
- Files: `*.integration.test.ts` in `packages/*/tests/`
