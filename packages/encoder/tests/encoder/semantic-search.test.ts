...
describe('semanticSearch', () => {
  ...
  describe('clear', () => {
    ...
    it('should be a no-op when VectorStore does not implement clear', async () => {
      ...
      const minimal: import('@pleaseai/soop-store/vector-store').VectorStore = {
        open: async () => {},
        close: async () => {},
        upsert: async () => {},
        remove: async () => {},
      ...
      const { MockEmbedding } = await import('@pleaseai/soop-encoder/embedding')
      const s = new (await import('@pleaseai/soop-encoder/semantic-search')).SemanticSearch({
        vectorStore: minimal,
